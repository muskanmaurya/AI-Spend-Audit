# AI Tool Pricing Sources & Data
*Data is accurate as of submission week (May 2026). All subscription prices are monthly per seat in USD unless marked as usage-based.*

### 1. Cursor
* **Hobby:** $0
* **Pro:** $20
* **Business:** $40
* **Enterprise:** Custom (Contact Sales)
* *Source:* [https://www.cursor.com/pricing](https://www.cursor.com/pricing)

### 2. GitHub Copilot
* **Individual:** $10
* **Business:** $19
* **Enterprise (Pro+):** $39
* *Source:* [https://github.com/features/copilot/plans](https://github.com/features/copilot/plans)

### 3. Claude (Anthropic)
* **Free:** $0
* **Pro:** $20
* **Max:** $100 (5x Usage Tier) / $200 (20x Usage Tier)
* **Team:** $30
* **Enterprise:** Custom (Contact Sales)
* **API direct:** Usage-based (Priced per 1M tokens)
* *Source:* [https://claude.com/pricing](https://claude.com/pricing)

### 4. ChatGPT (OpenAI)
* **Plus:** $20
* **Team:** $30 ($25 if billed annually)
* **Enterprise:** Custom (Contact Sales)
* **API direct:** Usage-based (Priced per 1M tokens)
* *Source:* [https://openai.com/chatgpt/pricing/](https://openai.com/chatgpt/pricing/)

### 5. Anthropic API Direct
* **API direct:** Usage-based
* *Source:* [https://www.anthropic.com/pricing](https://www.anthropic.com/pricing)

### 6. OpenAI API Direct
* **API direct:** Usage-based
* *Source:* [https://openai.com/pricing](https://openai.com/pricing)

### 7. Gemini (Google)
* **Pro (Advanced):** $20
* **Ultra:** Custom / Enterprise volume pricing
* **API:** Usage-based
* *Source:* [https://gemini.google.com/advanced](https://gemini.google.com/advanced)

### 8. Windsurf (Codeium)
* **Free:** $0
* **Pro:** $15
* **Team / Enterprise:** $25
* *Source:* [https://codeium.com/pricing](https://codeium.com/pricing)
